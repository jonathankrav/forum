...
public class AuthentificationFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		if (checkEndPoint(request.getMethod(), request.getServletPath())) {
			String sessoinId = request.getSession().getId();
			User user = securityContext.getUserBySessionId(sessoinId);
			if (user == null) {
				try {
					String[] credentials = getCredentials(request.getHeader("Authorization"));
					UserAccount userAccount = userRepository.findById(credentials[0])
							.orElseThrow(RuntimeException::new);
					if (!BCrypt.checkpw(credentials[1], userAccount.getPassword())) {
						throw new RuntimeException();
					}
					user = new User(userAccount.getLogin(), userAccount.getRoles());
					securityContext.addUserSession(sessoinId, user);
				} catch (Exception e) {
